...
class Settings(BaseModel):
    ...
    llm_provider: str = Field(
        default="anthropic",
        description='Which client to use: "anthropic" or "openai" (OpenAI-compatible).',
    )
    model: str = Field(default="claude-opus-4-8")
    api_key: Optional[str] = Field(
        default=None,
        description="API key/token. If None, the client reads the provider's env var.",
    )
    base_url: Optional[str] = Field(
        default=None,
        description=(
            "OpenAI-compatible base URL (e.g. a self-hosted gpt-oss-120b endpoint "
            "like 'http://host:8000/v1'). Only used when llm_provider='openai'."
        ),
    )
    ...
    embedding_provider: str = Field(
        default="auto",
        description=(
            'Embedding backend: "auto"/"sentence_transformers", "openai", or '
            '"hashing". "auto" prefers SentenceTransformers, falling back to hashing.'
        ),
    )
    embedding_model: str = Field(default="sentence-transformers/all-MiniLM-L6-v2")
    embedding_base_url: Optional[str] = Field(
        default=None,
        description="OpenAI-compatible base URL for embeddings (defaults to base_url).",
    )
    embedding_api_key: Optional[str] = Field(
        default=None, description="API key for the embeddings endpoint (defaults to api_key)."
    )
    ...
    def from_env(cls) -> "Settings":
        ...
        Recognized variables: ``DOC_AGENT_PROVIDER`` (``anthropic``/``openai``),
        ``DOC_AGENT_MODEL``, ``DOC_AGENT_BASE_URL``, ``DOC_AGENT_MAX_STEPS``,
        ``DOC_AGENT_ENABLE_RAG`` (``"0"``/``"false"`` to disable), ``DOC_AGENT_TOPK``,
        ``DOC_AGENT_WORKDIR``, ``DOC_AGENT_EMBED_PROVIDER``, ``DOC_AGENT_EMBED_MODEL``,
        ``DOC_AGENT_EMBED_BASE_URL``. The API key is read from ``DOC_AGENT_API_KEY``,
        else ``OPENAI_API_KEY`` / ``ANTHROPIC_API_KEY`` depending on the provider.
        Anything unset keeps its declared default.

        Returns:
            A validated :class:`Settings` instance.
        """
        kwargs: dict = {}
        provider = os.getenv("DOC_AGENT_PROVIDER")
        if provider:
            kwargs["llm_provider"] = provider
        if v := os.getenv("DOC_AGENT_MODEL"):
            kwargs["model"] = v
        if v := os.getenv("DOC_AGENT_BASE_URL"):
            kwargs["base_url"] = v
        ...
        if v := os.getenv("DOC_AGENT_EMBED_PROVIDER"):
            kwargs["embedding_provider"] = v
        if v := os.getenv("DOC_AGENT_EMBED_MODEL"):
            kwargs["embedding_model"] = v
        if v := os.getenv("DOC_AGENT_EMBED_BASE_URL"):
            kwargs["embedding_base_url"] = v

        # Provider-aware API key resolution.
        env_key_name = "OPENAI_API_KEY" if (provider == "openai") else "ANTHROPIC_API_KEY"
        api_key = os.getenv("DOC_AGENT_API_KEY") or os.getenv(env_key_name)
        if api_key:
