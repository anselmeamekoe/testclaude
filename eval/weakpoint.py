...
class WeakpointAnalyzer:
    ...
    def analyze(self, model, ds: Dataset) -> WeakpointReport:
        ...
        # neighbor structure for label-noise proxy (fit once on scaled test)
        scaler = StandardScaler().fit(Xte)
        nn = NearestNeighbors(n_neighbors=min(11, len(yte))).fit(scaler.transform(Xte))
        _, nbr_idx = nn.kneighbors(scaler.transform(Xte))
        ...
        for lid in np.unique(leaf_te):
            ...
            row = self._diagnose_slice(
                ...
                g_loss_te, g_loss_tr, g_score, noise_all, rules.get(lid, "all"))
            rows.append(row)
        ...
        verdict = self._verdict(slices, g_score, ds.task)
    ...
    # ---- per-slice diagnostics + attribution ---------------------------- #
    def _diagnose_slice(self, ds, model, m_te, m_tr, loss_te, loss_tr, yte, Xte,
                        g_loss_te, g_loss_tr, g_score, noise_all, rule) -> dict:
        n_te = int(m_te.sum())
        n_tr = int(m_tr.sum())
        ...
        # local overfit gap, scaled by the global train->test gap for context
        ...
        cause = self._attribute(loss_ratio, overfit_gap, local_loss_te, local_loss_tr,
                                g_loss_te, g_loss_tr, train_frac, coverage,
                                test_frac, noise)
        return dict(
            ...
            coverage=round(coverage, 2), label_noise=round(noise, 3),
            uncertainty=round(margin, 3), likely_cause=cause,
        )

    def _attribute(self, loss_ratio, overfit_gap, l_te, l_tr, g_te, g_tr,
                   train_frac, coverage, test_frac, noise) -> str:
        """Transparent rule set. Thresholds are relative to global behavior."""
        if loss_ratio < 1.25:
            return "(healthy)"
        # 1) local overfitting: test loss here much worse than TRAIN loss here,
        #    beyond the global train->test discrepancy.
        global_gap = g_te - g_tr
        if overfit_gap == overfit_gap and overfit_gap > max(2 * global_gap, 0.15) \
                and (l_tr < 0.8 * g_te):
            return "Local overfitting"
        # 2) sparsity / undersampled support
        if train_frac < 0.4 * test_frac or coverage < 0.5:
            return "Data sparsity"
        # 3) covariate shift: region much bigger at test time than train time
        if test_frac > 1.8 * train_frac:
            return "Distribution shift"
        # 4) irreducible: neighbors disagree a lot => high Bayes error
        if noise > 0.35:
    ...
    def _table_fig(self, slices: pd.DataFrame) -> go.Figure:
        show = slices[["rule", "n_test", "test_share", "train_share", "local_score",
                       "loss_ratio", "overfit_gap", "coverage", "label_noise",
                       "likely_cause"]].copy()
    ...
    def _verdict(self, slices, g_score, task) -> str:
        name = headline_name(task)
        weak = slices[slices["likely_cause"] != "(healthy)"]
        if weak.empty:
            return (f"No region materially worse than the global {name}={g_score:.3f}. "
                    "Error looks evenly spread — no obvious local/regional overfitting.")
        counts = weak["likely_cause"].value_counts().to_dict()
        worst = weak.iloc[0]
        parts = [f"Global {name}={g_score:.3f}. Found {len(weak)} weak region(s). "
