...
**Purpose.** Answer "is the data sufficient for this model?" with two modes that
go beyond a plain train/validation curve.

```python
LearningCurveAnalyzer(config: EvalConfig | None = None)
```

### `random_curve(model, ds, fractions=(0.1,0.2,0.35,0.5,0.7,0.85,1.0), repeats=3) -> LearningCurveReport`

Trains on increasing random subsets, then **fits an inverse power law to the
validation error and extrapolates**:

```
err(n) = c + a · n^(−b)
```

From the fit it derives actionable quantities: the asymptotic error `c` (the
error floor this model+data can reach), the error still on the table, and the
sample size needed to capture **90 % of the remaining gain**. It also labels the
regime.

**Regime labels (in `verdict`):**

| Regime | Signature | Recommendation |
|---|---|---|
| High-variance / data-limited | visible train–val gap, curve still descending | more data helps; the report gives the multiple of current `n` |
| Saturated | remaining error ≈ 0 | more rows won't help — raise capacity or add features |
| Mild returns | curve flattening | weigh collection cost vs. small expected lift |
| High-bias | train and val both low and close | underfit — more data alone won't fix it |

**`LearningCurveReport` (mode `"random"`)**

| Attribute | Contents |
|---|---|
| `table` | `quantity, value, note` — current scores, estimated ceiling, remaining error, `n` for 90 % of remaining gain |
| `fig` | plotly: train/val curves + extrapolation + ceiling line |
| `verdict` | regime label + numeric guidance |
| `details` | `curve` (DataFrame), `fit_ok, c, a, b, cur_err, floor, remaining, n_for_90pct` |

### `temporal_curve(model, ds, val_frac=0.2) -> LearningCurveReport`

**Drift-aware sufficiency** — the fraud-detection case, where older data can be
*detrimental*. Requires `time_col`. Orders the training pool by time, fixes the
validation block to the **most recent** `val_frac`, then grows the training
window **backward in time** (most-recent-first) and records validation score at
each window size. It also contrasts newest-`k` vs oldest-`k` equal blocks.

**Reading.** If validation peaks at a limited look-back `W*` and **decays** as
older history is added, old data is stale → cap the look-back at `W*` (or add
recency weighting / drift features). The `verdict` classifies **DRIFT DETECTED /
MILD DRIFT / STABLE**.
...
| Attribute | Contents |
|---|---|
| `table` | recency vs antiquity value, `W*`, score at `W*`, score using all history, cost of using all history |
| `fig` | plotly: validation vs window size, with `W*` marked |
| `details` | `curve` (DataFrame), `w_star, s_star, s_full, decay, s_new, s_old`, **`drift_drivers`** |

**Drift drivers** (`details["drift_drivers"]`). When drift is found, features are
ranked by how much their *marginal* distribution shifts between the oldest and
most-recent window, using three complementary metrics:

```
PSI          = Σ_i (p_i − q_i) · ln(p_i / q_i)      (K quantile bins)
KS           = sup_x |F_recent(x) − F_old(x)|
Wasserstein  = ∫ |F_recent(x) − F_old(x)| dx
```

**Important scope.** PSI/KS/Wasserstein detect **covariate** drift (feature
marginals moving). They are *blind to concept drift* where only P(y│x) changes
while the marginals stay put — the `temporal_curve`'s refit sweep is what catches
that. The verdict states this explicitly when covariate drift is weak but the
refit curve still shows decay. Cost: `O(d·n log n)`, no model calls.
...
| `temporal_curve` | ≈ `n_windows · F` (≈8 F) | yes |
...
| `random_curve` (power-law saturation & sample-size) | Cortes et al. 1993; Figueroa et al. 2012; Provost et al. 1999; Perlich et al. 2003; Hestness et al. 2017; Rosenfeld et al. 2020 |
| `temporal_curve` (drift / look-back window) | Gama et al. 2014; Widmer & Kubat 1996; Klinkenberg 2004; Dal Pozzolo et al. 2018; Tashman 2000; Bergmeir & Benítez 2012 |
...
| Drift drivers (PSI / KS / Wasserstein) | Lin 1991 (JSD/PSI); Massey 1951 (KS); Ramdas et al. 2017 & Villani 2009 (Wasserstein); Modeva docs (2025) |
