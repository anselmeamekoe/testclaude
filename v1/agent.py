...
class DocAgent:
    ...
    def __init__(
        ...
        Args:
            ...
            llm: LLM client; defaults to :class:`AnthropicLLM` bound to the
                configured model. Inject a fake here in tests.
            calibrator: Confidence calibrator; defaults to one built from settings
                with an identity Platt map.
        """
        self.settings = settings or Settings.from_env()
        self.backend = backend
        self.llm = llm or AnthropicLLM().with_model(self.settings.model)
    ...
    def _prepare_repository(self, payload: QuestionSet) -> None:
        ...
        embedder = build_embedder(self.settings.embedding_model)
    ...
    def _answer_one(self, item: QuestionItem) -> AgentAnswer:
        ...
        messages: list[dict[str, Any]] = [
            {"role": "user", "content": build_question_prompt(item)}
        ]
        ...
        for _ in range(self.settings.max_agent_steps):
            ...
            # Append the assistant turn verbatim (required for a valid transcript).
            if turn.raw_assistant_content:
                messages.append({"role": "assistant", "content": turn.raw_assistant_content})

            if not turn.tool_calls:
                # Model produced prose without calling submit_answer. Nudge once by
                # forcing the terminal tool on the next turn.
                ...
                if forced.raw_assistant_content:
                    messages.append(
                        {"role": "assistant", "content": forced.raw_assistant_content}
                    )
                turn = forced

            # Execute each requested tool and stage the tool_result blocks.
